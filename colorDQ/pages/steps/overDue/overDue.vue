<template>
	<view>
		<cu-custom bgColor="bg-red" :isBack="true"><block slot="backText">返回</block><block slot="content">逾期处理</block></cu-custom>
	...
	export default {
		...
		methods: {
			...
			dangerCommit(foo) {
				var self = this;
				uni.request({
					url: self.$store.state.urlPrefix + '/Process/Handle12',
					data: {
						YSCD: self.info.YSCD,
						BranchId: self.info.BranchId,
