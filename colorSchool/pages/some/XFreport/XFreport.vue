<template>
	<view id='yhsb'>
		...
		<view class="cu-form-group" id="sss">
			...
			<switch class='switch-sex red' @change="changeType" :class="isDanger?'checked':''" :checked="isDanger?true:false" color="#e54d42" style="font-size: 20upx;"></switch>
		</view>
			...
			getData() {
				...
				uni.request({
					...
					if (res.data.code == 0) {
						...
						self.name = reportInfo.Name;
						self.yscd = reportInfo.ID;
						let pcydArr = reportInfo.Detail;
						for (let s of pcydArr) {
							self.pcyd = self.pcyd + s.Name + ':' + s.Detail + ';'
						}
							...
							if (data.data) {
								...
								uni.showModal({
								    ...
								    success: function (res) {
								        if (res.confirm) {
								            uni.reLaunch({
								            	url: '/pages/home/dealList/dangerReview/dangerReview?YSCD=' + item.YSCD +
								            		"&StepOrder=" + item.StepOrder + "&BranchId=" +
								            		item.BranchId + "&No=" + item.No +
								            		"&AddUser=" + item.AddUser + "&CreateTime=" +
								            		item.CreateTime + "&JumpId=" + item.JumpId +
								            		"&navigatorId=" + navigatorId + "&ReceiveType=" + item.ReceiveType,
								            })
								        } else if (res.cancel) {
								            uni.reLaunch({
								            	url: '../../basic/home/home'
